...
public class ExamPaperController {
    ...
    /**
     * 预览自动生成的题目列表
     * @param rule 自动组卷规则（题型、难度、题量等）
     * @return 预览的题目列表（包含题目详情和建议分值）
     */
    @Operation(summary = "预览自动组卷结果", description = "依据题型、难度、题量等规则预览自动生成的题目列表，用户可以确认后再保存")
    @PostMapping("/preview-auto-generate")
    public Result<List<Map<String, Object>>> previewAutoGeneratedPapers(@Valid @RequestBody AutoGeneratePaperRule rule) {
        List<Map<String, Object>> questions = examPaperService.previewAutoGeneratedPapers(rule);
     ...
     * @param paperId 试卷ID
     * @param questionIds 题目ID列表
     * @param scores 对应题目的分值列表（可选，不提供则使用题目默认分值）
     * @return 保存结果
     */
    @Operation(summary = "保存自动组卷结果", description = "将预览确认的题目列表保存到试卷中")
